...
# 
# MPXV_cases_data_0<-subset(subset(MPXV_cases_data,is.na(Travel_history_country)),!is.na(Travel_history_location))#Artifically coding unknown country of travel to the bigger location"
...
df_count2 <- MPXV_cases_data %>% filter(Travel_history_country!='')  %>% count(Travel_history_country)
...
df_count2 <- df_count2 %>%
  left_join(MPXV_cases_data, by = c("Travel_history_country" = "Travel_history_country"))
...
mpxv_map <- ggplot() +
  ...
  scale_color_manual(
    values = c("mediumseagreen", "#8c510a", "#d8b365", "antiquewhite3", "grey70", "grey50", "grey20"),
